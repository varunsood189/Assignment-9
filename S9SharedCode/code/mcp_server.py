...
import demos as _demos  # noqa: E402
...
def list_demos() -> list[str]:
    """List canonical demo task ids (hello, hfmodels, aitools, laptops, …). Example: list_demos()."""
    return _demos.list_demos()


@mcp.tool()
def get_demo(name: str) -> dict:
    """Return the user query, expected DAG shape, and optional browser_hint for a demo id. Example: get_demo("hfmodels")."""
    try:
        return _demos.get_demo(name)
...
def get_hf_models_url(
    ...
    """Build a pre-filtered HuggingFace models URL (text-generation sorted by likes). Use when interactive filter clicks fail. Example: get_hf_models_url("text-generation", "likes")."""
    return _demos.get_hf_models_url(pipeline_tag=pipeline_tag, sort=sort)


@mcp.tool()
def get_cnc_browser_url(use_fallback: bool = False) -> dict:
    """Return the canonical Sulekha URL and goal for CNC/VMC institutes in Bangalore. Call when justdial.com failed or before first browser attempt on this demo. Example: get_cnc_browser_url()."""
    return _demos.get_cnc_browser_url(use_fallback=use_fallback)
