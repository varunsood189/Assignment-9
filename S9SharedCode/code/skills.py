...
# flow.py is often piped through `tee` in run_demo.sh; without line
# buffering Python holds stdout until ~8 KiB or process exit — looks frozen.
...
def _trim_upstream_output(output: dict | None, max_chars: int = 6_000) -> dict | None:
    """Cap long browser/researcher text so multi-input prompts stay bounded.

    Without per-field caps, one 60k browser page can consume the entire
    INPUTS JSON budget and hide sibling upstream nodes from the model."""
    if not isinstance(output, dict):
        return output
    trimmed = dict(output)
    for key in ("content", "findings", "final_answer", "summary"):
        val = trimmed.get(key)
        if isinstance(val, str) and len(val) > max_chars:
            trimmed[key] = val[:max_chars] + " … [truncated]"
    return trimmed
        ...
        if inp == "USER_QUERY":
            ...
            if isinstance(upstream, AgentResult):
                out.append({"id": inp, "kind": "upstream",
                            ...
                            "output": _trim_upstream_output(upstream.output)})
            else:
...
_TOOL_CATALOG = {
    ...
    "list_demos": {
        "name": "list_demos",
        "description": "List canonical demo task ids (hello, hfmodels, aitools, laptops, …).",
        "input_schema": {"type": "object", "properties": {}},
    },
    "get_demo": {
        "name": "get_demo",
        "description": (
            "Return query, expected DAG shape, and optional browser_hint "
            "for a demo id."
        ),
        "input_schema": {
            "type": "object",
    ...
    "get_hf_models_url": {
        "name": "get_hf_models_url",
        "description": (
            "Pre-filtered HuggingFace models listing URL (pipeline_tag + sort). "
            "Use when interactive HF filter clicks fail or hit step cap."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "pipeline_tag": {"type": "string", "default": "text-generation"},
                "sort": {"type": "string", "default": "likes"},
            },
        },
    },
    "get_cnc_browser_url": {
        "name": "get_cnc_browser_url",
        "description": (
            "Canonical Sulekha URL and goal for CNC/VMC institutes in Bangalore. "
            "Use for the cnc demo — avoid justdial.com (Playwright HTTP/2 failures)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "use_fallback": {"type": "boolean", "default": False},
            },
        },
    },
}
...
# MCP tools the planner may CALL during its turn — not orchestrator graph skills.
PLANNER_MCP_TOOLS = frozenset({
    "list_demos", "get_demo", "get_hf_models_url", "get_cnc_browser_url",
})


def _planner_fallback_plan(query: str, failure_report: str | None) -> dict | None:
    """Deterministic recovery when the planner model returns empty JSON."""
    import re
    from demos import get_cnc_browser_url

    q = (query or "").lower()
    fr = (failure_report or "").lower()

    # Critic rejected distiller — present existing fields via formatter (planner.md).
    if "critic" in fr and "distiller" in fr:
        m = re.search(r"target=(n:\d+)", failure_report or "")
        if m:
            did = m.group(1)
            return {
                "rationale": (
                    "Fallback: critic failed on distiller — formatter presents "
                    "distiller output and notes any ambiguity."
                ),
                "nodes": [
                    {
                        "skill": "formatter",
                        "inputs": ["USER_QUERY", did],
                        "metadata": {"label": "out"},
                    },
                ],
            }
    if "laptop" in q and any(x in q for x in ("80000", "80,000", "80k", "₹80")):
        return {
            "rationale": (
                "Fallback: three parallel researchers, one scoped question per "
                "laptop under ₹80k (canonical laptops demo shape)."
            ),
            "nodes": [
                {
                    "skill": "researcher",
                    ...
                    "metadata": {
                        "label": "r1",
                        "question": (
                            "Find the most popular laptop #1 under ₹80,000 in India: "
                            "model name, CPU, RAM, storage, display, price in INR, "
                            "one-line verdict on who it is best for."
                        ),
                    },
                },
                {
                    ...
                    "metadata": {
                        "label": "r2",
                        "question": (
                            "Find the most popular laptop #2 under ₹80,000 in India: "
                            "model name, CPU, RAM, storage, display, price in INR, "
                            "one-line verdict on who it is best for."
                        ),
                    },
                },
                {
                    ...
                    "metadata": {
                        "label": "r3",
                        "question": (
                            "Find the most popular laptop #3 under ₹80,000 in India: "
                            "model name, CPU, RAM, storage, display, price in INR, "
                            "one-line verdict on who it is best for."
                        ),
                    },
                ...
                {
                    "skill": "formatter",
                    "inputs": ["USER_QUERY", "n:r1", "n:r2", "n:r3"],
                    "metadata": {"label": "out"},
                },
            ],
        }
    if "cnc" in q and "bangalore" in q:
        use_fb = any(
            m in fr
            for m in ("justdial", "err_http2", "protocol_error", "browser")
        )
        hint = get_cnc_browser_url(use_fallback=use_fb)
        dq = hint["distiller_question"]
        return {
            "rationale": (
                "Fallback: browser on Sulekha listing after directory site "
                "failure or empty planner output."
            ),
            "nodes": [
                {
                    "skill": "browser",
                ...
                },
                {
                    "skill": "distiller",
                    "inputs": ["n:b1"],
                    "metadata": {"label": "d1", "question": dq},
                },
                {
                    "skill": "formatter",
                    "inputs": ["USER_QUERY", "n:d1"],
                    "metadata": {"label": "out"},
                },
            ],
        }
...
def _normalize_planner_node(spec_dict: dict) -> dict:
    """Drop mistaken MCP-tool nodes; ensure browser nodes have metadata.label."""
    out = dict(spec_dict)
    md = out.get("metadata")
    if not isinstance(md, dict):
        md = {}
        out["metadata"] = md
    if out.get("skill") == "browser" and not md.get("label"):
        md["label"] = "b1"
    return out


def _expand_critic_inputs(node_inputs: list[str], graph_nodes) -> list[str]:
    """Include distiller upstream sources so critic judges faithfulness to
    INPUTS, not world knowledge or USER_QUERY alone."""
    expanded = list(node_inputs)
    seen = set(expanded)
    for inp in node_inputs:
        if not inp.startswith("n:") or inp not in graph_nodes:
            continue
        if graph_nodes[inp].get("skill") != "distiller":
            continue
        for uinp in graph_nodes[inp].get("inputs") or []:
            if (uinp.startswith("n:") and uinp in graph_nodes
                    and uinp not in seen):
                expanded.append(uinp)
                seen.add(uinp)
    return expanded
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    node_inputs = list(graph_nodes[node_id]["inputs"])
    if skill.name == "critic":
        node_inputs = _expand_critic_inputs(node_inputs, graph_nodes)
    resolved = resolve_inputs(node_inputs, graph_nodes, query)
    ...
    if skill.name == "browser":
        ...
        label = meta.get("label") or meta.get("url", "?")
        print(f"[{node_id}] browser → {label}", flush=True)
        sk = BrowserSkill(
    ...
    if skill.name == "planner" and not (parsed.get("nodes") or []):
        fb = _planner_fallback_plan(query, failure_report)
        if fb:
            print("[skills] planner: empty model output — using demo fallback plan",
                  flush=True)
            parsed = fb

    if skill.name == "distiller" and parsed and not parsed.get("fields"):
        rat = parsed.get("rationale", "")
        if isinstance(rat, str) and rat.strip().startswith("{"):
            try:
                inner = json.loads(rat)
                if isinstance(inner, dict) and isinstance(inner.get("fields"), dict):
                    parsed = inner
            except json.JSONDecodeError:
                pass

    # Robustness fallback: some providers return plain text instead of JSON,
    # or truncate mid-response leaving an empty string. Map non-empty plain
    # text to the skill's primary output field so downstream nodes still get
    # something useful. For empty text, emit a sentinel so the Distiller can
    # report "not found" rather than silently returning fields: {}.
    if not parsed:
        node_meta = graph_nodes[node_id].get("metadata") or {}
        node_question = node_meta.get("question") if isinstance(node_meta, dict) else None
        q = node_question or query
        if skill.name == "researcher":
            findings = raw_text if raw_text else "(not found — model returned empty response)"
            sources: list[dict] = []
            if raw_text:
                try:
        ...
        elif skill.name == "formatter":
            parsed = {"final_answer": raw_text or "(no answer produced)"}
        elif skill.name == "distiller":
            parsed = {"fields": {}, "rationale": raw_text or "(distiller returned empty response)"}
            # Gemini sometimes nests the real payload inside rationale as JSON text.
            if raw_text and not parsed.get("fields"):
                try:
            ...
            parsed = {"summary": raw_text or "(not found)", "preserved_facts": []}
            ...
            if raw_text:
                ...
                parsed = {"verdict": "fail", "rationale": "(critic returned empty response)"}
        elif skill.name == "coder":
            parsed = {"code": raw_text or "", "rationale": "fallback from plain-text response"}
    ...
    if skill.name == "planner":
        for s in parsed.get("nodes", []) or []:
            if not isinstance(s, dict):
                rejected.append(f"node={s!r}  error=expected dict")
                continue
            if s.get("skill") in PLANNER_MCP_TOOLS:
                print(f"[skills] planner: dropped node with MCP tool name "
                      f"{s.get('skill')!r} — call tools during planning, "
                      f"do not emit them as graph skills")
                continue
