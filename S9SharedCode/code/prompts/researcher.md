...
Output rules:
  - Keep `findings` under 800 words. Summarise; do not paste raw page text.
  - Always emit the JSON object even if findings are thin. Never return
    an empty response — if nothing was found write `"findings": "(not found)"`.
  - The JSON must be the ONLY thing you output (no surrounding prose).
