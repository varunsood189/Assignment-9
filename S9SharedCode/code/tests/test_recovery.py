...
def test_critic_fail_cap_fires_on_second_failure_for_same_target() -> None:
    g = _StubGraph()
    g.g.add_node("n:b", skill="browser", status="complete", inputs=[], metadata={})
    g.g.add_node("n:t", skill="distiller", status="complete",
                 inputs=["n:b"], metadata={})
    g.g.add_node("n:c", skill="critic", status="complete",
                 inputs=["n:t"],
                 metadata={"target": "n:t", "child": "n:f"})
    g.g.add_node("n:f", skill="formatter", status="pending",
                 inputs=["n:c"], metadata={})
    g.g.add_edge("n:t", "n:c")
    g.g.add_edge("n:c", "n:f")
    recovered: dict[str, bool] = {"critic_branch:n:b": True}  # already recovered once
    cap: list[str] = []
    handled = handle_critic_verdict("n:c", _fail_result(), g, recovered, cap)
    assert handled is True
    assert cap == ["critic_branch:n:b"], "cap-hit should be surfaced for future logging"
    assert [a[1] for a in g._added] == [], "no second planner should be queued"
    assert g.g.nodes["n:f"]["status"] == "pending"
    assert g.g.nodes["n:f"]["inputs"] == ["USER_QUERY", "n:t"]
