    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as mcp:
            ...
            for _ in range(MAX_TOOL_HOPS + 1):
                # Gemini often returns 0 tokens on the post-tool turn when
                # tool_choice stays "auto" — force text/JSON completion instead.
                tool_choice = "none" if _has_tool_results(messages) else "auto"
                reply = await _chat(
                    messages=messages,
                    tools=tools_payload,
                    agent=agent,
                ...
                if not tool_calls:
                    if not (reply.get("text") or "").strip() and _has_tool_results(messages):
                        last_reply = reply = await _chat(
                            messages=messages,
                            tools=tools_payload,
                            agent=agent,
                            session_id=session_id,
...
async def _chat(*, messages, tools, agent, session_id, provider_pin,
                max_tokens, temperature, tool_choice="auto") -> dict:
